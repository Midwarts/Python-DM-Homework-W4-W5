...
def clear_space_in_polar(text):
    text = re.sub("^[ \xa0]*", "", text)
    text = re.sub("[ \xa0]*$", "", text)
    return text#清除前后空格(清除字符串前后两端的空白
    ...
    for page_num in range(10):
        ...
        for movie_label in bs.select("#content > div > div.article > ol > li"):  # 定位到电影标签
            ...
            rating_people = movie_label.select_one(
                "li > div > div.info > div.bd > div > span:nth-child(4)").text  # 获取评分人数
            ...
            country = clear_space_in_polar(info_line_2.split("/")[1]) if len(
                info_line_2.split("/")) >= 2 else None  # 提取电影国家
            classify = clear_space_in_polar(info_line_2.split("/")[2]) if len(
...
with open(r"C:\Python\PycharmProjects\数据抓取\豆瓣Top250榜单.json", "w+", encoding="UTF-8") as file:
